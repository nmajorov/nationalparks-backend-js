...
return  db[collection].createIndex({'coordinates':"2d"}, function(err, doc){
      var ncount=0
    if(err){
      console.log(err);
      //return db.close();
      ...
      //  return persist_db_connection || db.close();
        ...
        //  return db.close();
          ...
          db[collection].insert(points, function(errr,result){
            if(errr){
              console.log(errr);
            }else{
              db[collection].count(function(errr, ncount){

                console.log("Imported " + ncount);
...
function flush_db(persist_db_connection){
  ...
  db[collection].drop(  function(err,result){
    if(err){
      console.log(err);
    }
   //return persist_db_connection || db.close();
